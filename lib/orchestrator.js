...
import { loadAgent, buildSystemPrompt, hasPermission, matchesPatterns, loadAllAgents } from './agent-loader.js';
...
export class Orchestrator extends EventEmitter {
  ...
  createPermissionHandler(agent, sessionId, onDenial = null) {
    return async (toolName, input, options) => {
      ...
      // For Bash, check if it looks like a write command
      if (toolName === 'Bash' && input.command) {
        const cmd = input.command;
        const looksLikeWrite = /\b(echo|cat|printf|tee)\b.*>|>>|\bcp\b|\bmv\b|\bmkdir\b|\btouch\b/i.test(cmd);
        if (looksLikeWrite) {
          console.log(`[Orchestrator] Bash command appears to write files: ${cmd}`);
          return {
            behavior: 'deny',
            message: `Bash commands that write files are not allowed. This agent can only write via the Write tool to: ${(agent.permissions?.write || ['*']).join(', ')}`,
            interrupt: false
          };
        }
      }
  ...
  async *executeChatbotAgentStreaming(agent, agentPath, message, systemPrompt, context) {
    ...
    try {
      ...
      const { prompt: preparedPrompt, queryOptions: contextQueryOptions } =
        this.sessionManager.preparePromptWithContext(session, message, resumeInfo);
