...
export async function loadAgent(agentPath, vaultPath) {
  ...
  if (path.isAbsolute(agentPath)) {
    throw new Error(`Absolute agent paths are not allowed: ${agentPath}`);
  }

  // Security: Reject path traversal attempts
  const normalized = path.normalize(agentPath);
  if (normalized.startsWith('..') || normalized.includes('/../')) {
    throw new Error(`Path traversal not allowed: ${agentPath}`);
  }

  // Security: Verify the resolved path stays within the vault
  const fullPath = path.join(vaultPath, normalized);
  const resolvedFull = path.resolve(fullPath);
  const resolvedVault = path.resolve(vaultPath);

  if (!resolvedFull.startsWith(resolvedVault + path.sep) && resolvedFull !== resolvedVault) {
    throw new Error(`Agent path must be within the vault: ${agentPath}`);
  }
