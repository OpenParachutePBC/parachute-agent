...
export const AgentType = {
  CHATBOT: 'chatbot',   // Persistent conversation - maintains session
  WORKFLOW: 'workflow'  // One-shot execution - no session
};
 ...
 *
 * @param {string} agentPath - Path to the agent markdown file
...
export async function loadAgent(agentPath, vaultPath) {
  ...
  const agent = {
    ...
    // Agent type - chatbot (conversation) or workflow (one-shot)
    ...
    // Permissions
    permissions: {
      ...DEFAULT_AGENT_CONFIG.permissions,
...
The vault path is the current working directory. Use the relative path: ${context.documentPath}`;
  ...
  if (agent.permissions.write && agent.permissions.write[0] !== '*') {
    prompt += `\n\n## Write Permissions\n`;
    prompt += `You can only write to these locations:\n`;
